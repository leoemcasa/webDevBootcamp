...
app.get("/jokes/:id", (req, res) => {
  ...
  const joke = jokes.find((e) => e.id === reqId);
  res.json(joke);
});
...
app.get("/filter", (req, res) => {
  ...
  const filterdJokes = jokes.filter((e) => e.jokeType === filter);
...
app.patch("/jokes/:id", (req, res) => {
  const id = parseInt(req.params.id);
  const existingJoke = jokes.find((e) => e.id === id);
  const replaceJoke = {
    id: id,
    jokeText: req.body.text || existingJoke.jokeText,
    jokeType: req.body.type || existingJoke.jokeType,
  };
  const searchIndex = jokes.findIndex((e) => e.id === id);
  jokes[searchIndex] = replaceJoke;
...
app.delete("/jokes/:id", (req, res) => {
  ...
  if (searchIndex > -1) {
    ...
    res
      .status(404)
      .json({ error: `Joke Id: ${id} not found. No Joke deleted`});
  }
});
...
app.delete("/all", (req, res) => {
  ...
  if (userKey === masterKey) {
    ...
    res
      .status(404)
      .json({ error: `You are note authorized to.` })
  }
});
